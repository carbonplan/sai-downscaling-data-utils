...
| [`data_access.py`](data_access.py) | Reading the downscaled and bias-corrected data: which store to open, which members and variables a scenario contains, coverage, loading a group, request sizes and output filenames. | `download.py`, `input_access.py`, and the [`quickstart`](../notebooks/quickstart.ipynb), [`subsetting-and-exporting`](../notebooks/subsetting-and-exporting.ipynb) and [`compute-resources`](../notebooks/compute-resources.ipynb) notebooks |
...
The quickstart, subsetting and compute-resources notebooks import their helper functions from `data_access.py` and `notebook_helpers.py`, so a change made in the module reaches them after a kernel restart. The exception is [`input-data.ipynb`](../notebooks/input-data.ipynb), which carries its own copy of `input_access.py` so it can run on its own; a change to either one needs the same change in the other.
